...
- Seguir con `assembler.py` v0.5 para snake/examples (verificado punta a punta).
- Si se migra a `rtm32.asm` v1.2.0: bajar el build `x86_64-linux-musl`,
  renombrar a `rtm32.asm`, y reescribir los fuentes a sintaxis `.rtm`.
- Renombrar `aarch64-linux-musl-rtm32.asm` no sirve en esta PC; guardarlo solo
  como referencia o borrarlo (ocupa 386 KB).
